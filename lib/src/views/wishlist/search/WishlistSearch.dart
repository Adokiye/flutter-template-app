...
import 'package:Instahelp/src/providers/request_services/Api+search.dart';
import 'package:Instahelp/src/views/home/search/SearchResultCell.dart';

class WishlistSearchPage extends StatefulWidget {
  // Callback events
  final List<City> cities;

  const WishlistSearchPage({Key key, this.cities}) : super(key: key);

  @override
  _WishlistSearchPageState createState() => _WishlistSearchPageState();
}

class _WishlistSearchPageState extends State<WishlistSearchPage> {
  ...
  Future search(String text) async {
    return ApiSearch.searchPlaces(text).then((response) {
      _searchPlaces = List<Place>.generate(
        ...
        (i) {
          var p = Place.fromJson(response.data[i]);          
          return p;
        });
        setState(() {
        });
