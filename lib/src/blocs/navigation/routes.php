...
$router->group([
    ...
    $router->get('/channel/downloadFile/{id}', 'ChatController@downloadFile');
    $router->get('/channel/{id}/read', 'ChatController@read');
});
$router->group([
    'prefix' => 'app',
    'namespace' => 'API',
    ...
    $router->get('/user/payment-info', 'PaymentController@index')->name('payment_info')->middleware('auth');
    $router->post('/user/payment-info/card/new', 'PaymentController@addCard')->name('add_new_card')->middleware('auth');
    $router->post('/user/payment-info/card/default', 'PaymentController@setDefaultPayment')->name('set_default_card')->middleware('auth');
    $router->post('/user/payment-info/card/remove', 'PaymentController@removeCard')->name('remove_card')->middleware('auth');
    $router->post('/user/payment-info/subscription/subscribe', 'PaymentController@subscribe')->name('subscribe')->middleware('auth');
    $router->post('/user/payment-info/subscription/cancel', 'PaymentController@cancelSubscription')->name('unsubscribe')->middleware('auth');
    $router->post('/user/payment-info/subscription/resume', 'PaymentController@resumeSubscription')->name('resume')->middleware('auth');
    /**
     * Places
     */
